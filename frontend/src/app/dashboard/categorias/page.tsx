...
export default function CategoriasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await categoriasApi.atualizar(editing.id, form);
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      await categoriasApi.deletar(item.id);
  ...
  return (
    <PageHeader
      title="Categorias"
      subtitle="Organização dos materiais por tipo"
      actions={
        <button
          onClick={() => { setForm({ nome: "", descricao: "" }); setEditing(null); setShowForm(!showForm); }}
        ...
        <form onSubmit={handleSubmit} className="glass p-6 mb-6 animate-fade-in">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} required className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg text-white text-sm focus:ring-2 focus:ring-emerald-500/50 focus:outline-none" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-1">Descrição</label>
              <input value={form.descricao} onChange={(e) => setForm({ ...form, descricao: e.target.value })} className="w-full px-3 py-2 bg-slate-700/50 border border-slate-600/50 rounded-lg text-white text-sm focus:ring-2 focus:ring-emerald-500/50 focus:outline-none" />
            </div>
          </div>
